import torch
from transformers import AutoModelWithLMHead, AutoTokenizer,AutoModelForTokenClassification, AutoModelForCausalLM
...
class Interaction():
    ...
    def __init__(self, args):
        ...
        self.topic_cls = BertClassificationPredictor(
            ...
            label_path=args['label_dir'].replace('"',''),  #sys.argv[2], # directory for labels.csv file
            multi_label=False,
            model_type='bert',
            do_lower_case=True)
        ...
        self.conv_line.cuda()
        self.conv_line.eval()
        self.baseline_tokenizer = AutoTokenizer.from_pretrained("/nas/home/zixiliu/dialogsystem_model/baseline")
        self.baseline_model = AutoModelForCausalLM.from_pretrained("/nas/home/zixiliu/dialogsystem_model/baseline")
    ...
    def baseline_decode(self, user_utt):
        ...
        new_user_input_ids = self.baseline_tokenizer.encode(
            user_utt + self.baseline_tokenizer.eos_token, return_tensors='pt', max_length=128).cuda()
        if user_utt=="BEGIN":
            np.random.seed(random.randint(0,120))
            torch.manual_seed(random.randint(0,120))
            chat_history_ids = self.baseline_model.generate(
                new_user_input_ids, max_length=60, top_k=10, top_p=0.70, pad_token_id=self.baseline_tokenizer.eos_token_id)
            utterance = self.baseline_tokenizer.decode(chat_history_ids[0], skip_special_tokens=True)
    ...
    def get_entities(self, utterance):
        ...
        inputs = self.entity_ext_tokenizer.encode(utterance, return_tensors="pt").to('cpu')
    ...
    def get_response_keywords(self, utterance, topic, entities, randomness=False):
        '''
    ...
    def get_response(self, user_utterance, user_utt_topic, res_keywords):
        ...
        if self.gen_model_type =='dialogpt':
            ...
            with torch.no_grad():
                hypotheses = self.gen_model.sample(slines, sampling=True, sampling_topk=self.top_k ,temperature=self.temperature ,lenpen=2.0, max_len_b=maxb, min_len=minb, no_repeat_ngram_size=3)
            hypotheses = hypotheses[0]
            response = hypotheses.replace('\n','')
        return response
...
if __name__=="__main__":
    ...
    parser['gen_model_path'] = '/nas/home/zixiliu/dialogsystem_model/bartgen/'
    parser['conv_line_path'] = '/nas/home/zixiliu/dialogsystem_model/ent_kwd/'
    parser['topic_cls_path'] = '/nas/home/zixiliu/dialogsystem_model/topic_cls/'
    parser['label_dir'] = '/nas/home/zixiliu/dialogsystem_model/topic_cls/'
    parser['length'] = 100
    ...
    interaction = Interaction(parser)
    print('Hello, Welcome! I am happy to chat with you :)' )
    prev1 = ''
    prev2 = 'Hello, Welcome! I am happy to chat with you :)'
    while True:
        user_utt = input('USER: ')
        if user_utt.lower() == 'exit':
            break
        prev_utts = '[CLS]'+prev1+'[SEP]'+prev2+'[SEP]'+user_utt+'[SEP]'
        user_utt_topic = interaction.get_topic(prev_utts)
        ...
        print("Response keywords are", res_utt_keywords)
        # user_reposnse_edit = input('Edit response keywords: ')
        # res_utt_keywords = user_reposnse_edit.split(' # ')
        # print("Response keywords are", res_utt_keywords)
        ...
        prev1 = copy.copy(user_utt)
        prev2 = copy.copy(sys_utt)
